...
export const addToCartItemController = async (request, response) => {
    try {
        ...
        // Added variant fields to match exact values
        // For empty/null values, using $in to match null, empty string, or using $or for $exists: false
        if (normalizedSize !== null) {
            query.size = normalizedSize;
        } else {
            // Matching null, empty string, or undefined for products without size
            query.$or = [
                { size: null },
                { size: '' },
                { size: { $exists: false } }
